...
public class ServiceTask {
    ...
    public boolean addOpportunite(Opportunite t) {
        ...
        return resultOK;
         ...
         public boolean UpdateOpportunite(Opportunite t,int id) {
                String url = "http://127.0.0.1:8000/UpdateOpportunite?id="+id+"&&titre="+t.getTitre()+"&&lieu="+t.getLieu()+"&&desc="+t.getDescription()+"&&nom_ent="+t.getNom_entreprise()+"&&taille="+t.getTaille_entreprise()+"&&poste="+t.getPoste()+"&&media="+t.getMedia()+"&&recru="+t.getNombre_recrutement();
                req.setUrl(url);// Insertion de l'URL de notre demande de connexion        
        //req.setPost(true);
        req.addResponseListener(new ActionListener<NetworkEvent>() {
            @Override
 ...
 public boolean addCandidature(Candidature t) {
                String url = "http://127.0.0.1:8000/addCandidature/"+t.getTitre_id()+"?fonction="+t.getFonction()+"&&type_contrat="+t.getType_contrat()+"&&horaires="+t.getHoraires()+"&&mode_salaire="+t.getMode_salaire()+"&&periode="+t.getPeriode()+"&&annuel_mois="+t.getAnnuel_mois();
                req.setUrl(url);// Insertion de l'URL de notre demande de connexion        
        //req.setPost(true);
        req.addResponseListener(new ActionListener<NetworkEvent>() {
            @Override
 ...
 public boolean DeleteCandidature(int identif) {
                String url = "http://127.0.0.1:8000/deleteCand/"+identif;
                req.setUrl(url);// Insertion de l'URL de notre demande de connexion        
        //req.setPost(true);
        req.addResponseListener(new ActionListener<NetworkEvent>() {
            @Override
